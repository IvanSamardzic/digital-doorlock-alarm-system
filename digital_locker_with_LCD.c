...
void get_key(void){
	...
	//get_key() function  is uset to read the pressed key and this information is 
	...
	for(column = 1, temp = 1; column <= 4; temp *= 2, column++){
		...
		if(key > 0){
			...
			while((PINB & 0xF0) != 0); //wait for keyrelease
	...
	for(digit = 4; digit >= 1; digit--){
		if(number[digit-1] != 10){
			if(show == 1){
				...
				LCD_blink(); //blink LCD screen if show option equals zero
			}
		}
	}
}
...
void block_time(){
	...
	if(wait == 10){ //blocking time = 10 sec, it can be up to 65,535 seconds
		...
		//if blocking time is not completed
		EEPROM_write(11, wait % 256); //update EEPROM register
...
int main(void){
	...
	PORTC |= 0x40; //wait LED is at HIGH state
	...
	if(block == 1){
		...
		PORTC &= 0xBF; //wait LED is at LOW state
		PORTC |= 0x20; //blocked LED is at HIGH state
		...
		PORTC &= 0x1F; //ready LED, wait LED and blocked LED are at LOW state
		PORTC |= 0x80; //ready LED is at HIGH state
	}
